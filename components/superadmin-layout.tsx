...
export function SuperAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar */}
      <div className="w-64 bg-gradient-to-b from-red-900 to-red-950 text-white">
        <div className="p-6 border-b border-red-800">
          <div className="flex items-center gap-2">
            <Shield className="h-8 w-8" />
            <div>
              <h1 className="text-xl font-bold">Super Admin</h1>
              <p className="text-xs text-red-200">ProprioFinder</p>
            </div>
          </div>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 w-64 p-4 border-t border-red-800">
          <Link
            href="/dashboard"
            className="flex items-center gap-2 px-4 py-2 text-sm text-red-200 hover:text-white transition-colors"
          >
